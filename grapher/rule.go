...
func (r *GraphUnitRule) Recipes() []string {
	return []string{
		fmt.Sprintf("%s tool %s %q %q < $< | src internal normalize-graph-data --unit-type %q --dir . 1> $@", srclib.CommandName, r.opt.ToolchainExecOpt, r.Tool.Toolchain, r.Tool.Subcmd, r.Unit.Type),
	}
}
